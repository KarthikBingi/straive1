...
def parse_queries_from_text(text):
    queries = {}
    # Accepts both "-- Query1" and "Query1"
    ...
    if not uploaded_file:
        ...
        try:
            ...
            if not queries:
                ...
                st.info(f"Executing {len(queries)} queries in IST timezone...")
                excel_data = execute_queries_with_ist(db_user, db_pass, db_host, db_port, db_service, queries, start_time, end_time)
                filename = f"db_results_IST_{datetime.now().strftime('%Y%m%d_%H%M%S')}.xlsx"
                st.success("✅ Queries executed successfully with IST conversion!")

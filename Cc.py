...
if connect_btn:
    try:
        ...
        # Convert Oracle timezone string to numeric offset
        # Example: "-04:00" or "+05:30"
        hours, mins = map(int, db_timezone.split(":"))
        db_timezone_offset = timezone(timedelta(hours=hours, minutes=mins))
...
start_time_str = st.text_input("Start Time (YYYY-MM-DD HH24:MI:SS)")
end_time_str = st.text_input("End Time (YYYY-MM-DD HH24:MI:SS)")
...
def parse_queries_from_text(text: str):
    ...
    """
    queries = {}
    parts = re.split(r"(?i)--\s*Query\d+|(?i)\bQuery\d+\b", text)
    query_names = re.findall(r"(?i)--\s*Query\d+|(?i)\bQuery\d+\b", text)

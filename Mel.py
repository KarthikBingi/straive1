...
if st.button("Connect to Database"):
    try:
        ...
        # Mapping timezone offsets to readable labels
        timezone_map = {
...
def send_email(recipient, file_path):
    ...
    try:
        with smtplib.SMTP("smtp.office365.com", 587) as server:
            server.starttls()
            server.login(sender_email, password)  # Use the same DB password or ask separately
    ...
    if mode == "Single Query":
        ...
        if st.button("Run Query"):
            try:
                ...
                st.download_button("⬇️ Download Excel", data=open(file_path, "rb"), file_name="single_query_output.xlsx")
        ...
        if st.button("Execute SQLs"):
            if not sql_file:
                ...
                try:
                    sqls = [line.strip().rstrip(';') for line in sql_file.readlines() if line.strip() and not line.strip().startswith("--")]
                    ...
                    st.download_button("⬇️ Download Excel", data=open("output.xlsx", "rb"), file_name="output.xlsx")
